...
(function() {
		...
		function showData(index,loadNext)
		{
			...
			if(loadNext == true)
			{
				...
				if(($cycleIdArr.indexOf(cycleId)+2) >= $cycleIdArr.length)
				{
					...
					//console.log(($cycleIdArr.indexOf(cycleId)+2)+" Setting 2 from If "+$cycleIdArr.length)
				...
				eventObjToSend["moduleScore"] = $apiModuleScore;
				eventObjToSend["moduleStatus"] = apiModuleStatus;
				eventObjToSend["cycleNum"] = (index+1);
				eventObjToSend["cycleScore"] = cumulativeScore;
				eventObjToSend["cycleAttemptNumber"] = passCount;

				var paramObj = [];
				for (var key in $apiCompetencyData) 
				{
					var score = $apiCompetencyData[key];
					var tempObj = [];
					tempObj[key] = score;
					paramObj[paramObj.length] = tempObj;
				}
				
				eventObjToSend["parameterList"] = paramObj;

				//console.log("Sending to API "+eventObjToSend);
		...
		function loadTemplateCss()
		{
			...
			link.rel  = 'stylesheet';
			link.type = 'text/css';
			link.href = "pages/templates/feedback/feedback.css";
			link.media = 'all';
